...
- Dagster can materialize the output of a op to persistent storage. [IOManagers](/concepts/io-management/io-managers) let us separate business logic from IO, which lets us write code that's more testable and portable across environments.
